...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_COPY_CARD] = "images\\Cpy2.jpeg";
	MenuItemImages[ITM_CUT_CARD] = "images\\Cut_Card2.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_ROLL_DICE] = "images\\Dice.jpeg";
	...
	MenuItemImages[ITM_NEW_GAME] = "images\\New_Game.jpeg";
	//MenuItemImages[ITM_EXIT] = "images\\Exit.jpg";
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	//       (Use GetIntegerSize() window function) and set the "w" and "h" variables with its width and height
	pWind->GetIntegerSize(w, h, cellPos.GetCellNum());
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int y_12 = GetCellStartY(fromCell);

	for (int i = 0; i < fromCell.VCell() - toCell.VCell(); i++) {
		pWind->DrawLine(x12, y_12, x34, y_12);
		y_12 -= UI.CellHeight;
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int x_value[4], y_value[4];
	x_value[0] = x12;
	y_value[0] = y1 + yChange;
	x_value[1] = x12 + xChange;
	y_value[1] = y1;
	x_value[2] = x12;
	y_value[2] = y1 - yChange;
	x_value[3] = x12 - xChange;
	y_value[3] = y1;

	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way
	pWind->DrawPolygon(x_value, y_value, 4);
